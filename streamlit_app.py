...
# --- 1. LOGIQUE DU BACKEND (INCHANGÉE) ---
...
def produce_diagnostic(user_answers: Dict[str, str]):
    diagnostic_profile = {}
    answer_map = {ans.id: ans for q in questionnaire_db.questions for ans in q.answers}
    for key, answer_id in user_answers.items():
        if key.endswith(("_text", "_slider", "_conditional_text")): continue
        if answer_id in answer_map:
            answer_obj = answer_map[answer_id]
            for typologie, categories in answer_obj.scoring.items():
                if typologie not in diagnostic_profile: diagnostic_profile[typologie] = {}
...
st.markdown(f"""<div class="header"><h1>{questionnaire_db.title}</h1><p>{questionnaire_db.introduction}</p></div>""", unsafe_allow_html=True)
        ...
        for i, q in enumerate(questions_in_section):
            with st.container():
                if q.input_type == 'text_input':
                    user_answers[q.id] = st.text_input(q.text, key=q.id, placeholder="Votre réponse...")
                elif q.input_type == 'radio':
                    answer_options = [ans.text for ans in q.answers]
                    answer_text_to_id = {ans.text: ans.id for ans in q.answers}
                    selected_option = st.radio(q.text, options=answer_options, key=q.id, horizontal=True)
                    if selected_option: user_answers[q.id] = answer_text_to_id[selected_option]
                elif q.input_type == 'grid_choice':
                    st.markdown(f"**{q.text}**")
                    if q.rows and q.answers:
                        col_map = {ans.text: ans.id for ans in q.answers}
                        for row in q.rows:
                            selected = st.radio(f"*{row['text']}*", options=[a.text for a in q.answers], key=f"{q.id}_{row['id']}", horizontal=True)
                            if selected: user_answers[f"{q.id}_{row['id']}"] = col_map[selected]
                elif q.input_type == 'checkbox':
                    st.markdown(f"**{q.text}**")
                    for answer in q.answers:
                        if st.checkbox(answer.text, key=answer.id): user_answers[answer.id] = answer.id
                elif q.input_type == 'radio_with_other':
                    options = [ans.text for ans in q.answers]
                    text_map = {ans.text: ans.id for ans in q.answers}
                    selected = st.radio(q.text, options=options, key=q.id)
                    if selected:
                        user_answers[q.id] = text_map[selected]
                        if selected == "Autre":
                            other_text = st.text_input("Veuillez préciser :", key=f"{q.id}_other_text")
                            if other_text: user_answers[f"{q.id}_other_text"] = other_text
                elif q.input_type == 'radio_with_conditional_text':
                    options = [ans.text for ans in q.answers]
                    text_map = {ans.text: ans.id for ans in q.answers}
                    selected_text = st.radio(q.text, options=options, key=q.id, horizontal=True)
                    if selected_text:
                        selected_id = text_map[selected_text]
                        user_answers[q.id] = selected_id
                        if selected_id in q.conditional_config.get('trigger_answers', []):
                            prompt = q.conditional_config.get('prompt_text', "Veuillez préciser :")
                            cond_text = st.text_input(prompt, key=f"{q.id}_conditional")
                            if cond_text: user_answers[f"{q.id}_conditional_text"] = cond_text
                elif q.input_type == 'skin_tone_selector':
                    st.markdown(f"**{q.text}**")
                    st.info("Ta couleur naturelle n'est pas un teint, c'est ton niveau réel de mélanine...")
                    user_answers[f"{q.id}_slider"] = str(st.slider("Comment décrirais-tu ton teint ?", 1, 10, key=f"{q.id}_slider"))
                    if q.answers:
                        if st.checkbox(q.answers[0].text, key=q.id): user_answers[q.id] = q.answers[0].id
            if i < len(questions_in_section) - 1: st.markdown("<hr style='margin: 1.5rem 0; border-color: #F0F2F6;'>", unsafe_allow_html=True)
